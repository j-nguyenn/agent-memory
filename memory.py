...
with (
    ...
    # Delete any existing memories for this scope to start fresh
    project_client.memory_stores.delete_scope(name=memory_store.name, scope=scope)
    print(f"Cleared existing memories for scope '{scope}'")
    ...
    update_poller = project_client.memory_stores.begin_update_memories(
        ...
        scope=scope,
        items=[user_message, new_message],  # Pass conversation items that you want to add to memory
        update_delay=0,  # Trigger update immediately
    )
    print(f"Scheduled memory update operation (Update ID: {update_poller.update_id}, Status: {update_poller.status()})")

    update_result = update_poller.result()
    print(
        f"Update {update_poller.update_id} completed with {len(update_result.memory_operations)} memory operations"
    )
